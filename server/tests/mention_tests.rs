...
/// (token, user_id).
async fn register_member(app: axum::Router, username: &str, server_id: &str) -> (String, String) {
...
async fn mention_username_resolved() {
    ...
    let (_, alice_id) = register_member(app.clone(), &alice_name, &sid).await;
...
async fn multiple_username_mentions() {
    ...
    let (_, alice_id) = register_member(app.clone(), &alice_name, &sid).await;
    let (_, bob_id) = register_member(app.clone(), &bob_name, &sid).await;
