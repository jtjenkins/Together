...
describe('serverStore', () => {
  ...
  describe('createServer', () => {
    it('should create and add a server', async () => {
      ...
      const result = await useServerStore.getState().createServer({ name: 'New Server' });
  ...
  describe('updateServer', () => {
    it('should update a server in the list', async () => {
      ...
      await useServerStore.getState().updateServer('server-1', { name: 'Updated Server' });
  ...
  describe('setActiveServer', () => {
    it('should set active server and clear members', () => {
      ...
      useServerStore.setState({
        members: [{ user_id: 'u1', username: 'test', avatar_url: null, status: 'online', nickname: null, joined_at: '' }],
      });
  ...
  describe('updateMemberPresence', () => {
    it('should update member status', () => {
      useServerStore.setState({
        members: [
          { user_id: 'u1', username: 'test', avatar_url: null, status: 'online', nickname: null, joined_at: '' },
        ],
      });
