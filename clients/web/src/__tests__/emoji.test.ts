...
import { parseEmoji, searchEmoji, EMOJI_CATEGORIES } from "../utils/emoji";
...
describe("parseEmoji", () => {
  ...
  it("replaces a known :name: alias", () => {
    // +1 is an alias for thumbsup
  ...
  it("converts :snowflake: correctly", () => {
    expect(parseEmoji(":snowflake:")).toBe("❄️");
  });
...
describe("searchEmoji", () => {
  ...
  it("respects the limit parameter", () => {
    // Empty query matches everything; limit to 3
    const results = searchEmoji("", 3);
    expect(results).toHaveLength(3);
  });

  it("returns at most the default limit of 20 for a broad query", () => {
    const results = searchEmoji("a"); // many names contain 'a'
    expect(results.length).toBeLessThanOrEqual(20);
  });

  it("is case-insensitive — uppercase query matches lowercase name", () => {
    const results = searchEmoji("THUMBSUP");
    const found = results.find((r) => r.name === "thumbsup");
    expect(found).toBeDefined();
  });

  it("returns results that each have emoji and name fields", () => {
...
describe("EMOJI_CATEGORIES", () => {
  ...
  it("every emoji entry has an emoji character and a name", () => {
    for (const cat of EMOJI_CATEGORIES) {
  ...
  it("contains the thumbsup entry with +1 as an alias in People", () => {
  ...
  it("contains the Smileys category with joy (😂)", () => {
